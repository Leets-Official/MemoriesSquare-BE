...
import leets.memoriessquare.domain.user.usecase.UserRefreshToken;
...
public class UserController {
    ...
    @Operation(summary = "AccessToken 갱신", description = "쿠키에 저장된 RefreshToken으로 AccessToken을 갱신합니다.")
    @ApiResponses({
            @ApiResponse(responseCode = "200", content = @Content(schema = @Schema(implementation = Boolean.class))),
            @ApiResponse(responseCode = "401", content = @Content(schema = @Schema(implementation = ErrorResponse.class))),
            @ApiResponse(responseCode = "403", content = @Content(schema = @Schema(implementation = ErrorResponse.class))),
    ...
    public JwtResponse refresh(@Parameter(hidden = true) @CookieValue("refreshToken") String refreshToken) {
        return userRefreshToken.execute(refreshToken);
    }
}
